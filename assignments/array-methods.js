...
let allCaps = runners.map(runner => `${runner.first_name.toLocaleUpperCase()} ${runner.last_name}`);
console.log(allCaps); 
...
// Problem 1
// The event director wants to find out which runners donated $100 or more so they can send them a thank you email. Get a list of runners' email addresses who donated $100 or more.
let emailList = runners.filter(runner => runner.donation >= 100).map(runner => runner.email);
console.log(emailList);

// Problem 2
// The event director wants to find a total amount of donations based on all runners who ordered a Medium sized shirt. Get a total on donations for all runners who order a Medium sized shirt.
let donationMediumTotal = runners.filter(runner => runner.shirt_size === 'M').reduce((total, runner) => total + runner.donation, 0);
console.log(donationMediumTotal);

// Problem 3
// The event director wants a list of companies that the runners are associated with and their donation amount. Get a list of all companies and donation amounts and log it in alphabetical order.
let companyList = runners.map(runner => `${runner.company_name}: \$${runner.donation}`).sort();
